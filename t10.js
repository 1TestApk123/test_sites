...
(function() {
    ...
    dwr.engine.defaultPollStatusHandler = function(newStatus, ex) {
        if (newStatus === false && dwr.engine._errorHandler) {
    ...
    dwr.version = {
        ...
        build: parseInt("514", 10),
        title: "RC3-dev",
        label: "3.0.0-RC3-dev-514"
    };
    ...
    dwr.engine._initializer = {
        ...
        preInit: function() {
            dwr.engine._pageId = dwr.engine.util.tokenify(new Date().getTime()) + "-" + dwr.engine.util.tokenify(Math.random() * 10000000000000000);
    ...
    dwr.engine._handleTextHtmlResponse = function(batch, textHtmlObj) {
        if (batch && typeof batch.textHtmlHandler === "function") {
            batch.textHtmlHandler(textHtmlObj)
        } else {
            if (dwr.engine._textHtmlHandler) {
                dwr.engine._textHtmlHandler(textHtmlObj)
            } else {
                dwr.engine._handleTextOrRedirectResponse(batch, textHtmlObj)
            }
        }
        if (batch) {
            dwr.engine.batch.remove(batch)
    ...
    dwr.engine._handleTextOrRedirectResponse = function(batch, textHtmlObj) {
        if (batch && typeof batch.textOrRedirectHandler == "function") {
            batch.textOrRedirectHandler(textHtmlObj)
        } else {
            if (dwr.engine._textOrRedirectHandler) {
                dwr.engine._textOrRedirectHandler(textHtmlObj)
            }
        }
        if (batch) {
            dwr.engine.batch.remove(batch)
        ...
        try {
            if (window.console) {
                ...
                if (window.opera && window.opera.postError) {
                    ...
                    if (window.Jaxer && Jaxer.isOnServer) {
    ...
    dwr.engine.remote = {
        ...
        handleException: function(batchId, callId, ex) {
            ...
            if (typeof handlers.exceptionHandler == "function") {
                ...
                if (typeof batch.errorHandler == "function") {
                    batch.errorHandler(ex.message, ex)
                }
            }
        },
    ...
    dwr.engine.serialize = {
                                ...
                                if (objstr == "[object Number]") {
                                    ...
                                    if (objstr == "[object Date]") {
                                        ...
                                        if (objstr == "[object Array]") {
                                            batch.map[name] = dwr.engine.serialize.convertArray(batch, directrefmap, otherrefmap, data, name, depth + 1)
                                        } else {
                                            if (data && data.tagName && data.tagName.toLowerCase() == "input" && data.type && data.type.toLowerCase() == "file") {
        ...
        getObjectClassName: function(obj) {
            if (obj.$dwrClassName) {
                ...
                if (obj.constructor && obj.constructor.$dwrClassName) {
    ...
    dwr.engine.transport = {
        send: function(batch) {
            ...
            if (!dwr.engine._dwrSessionId) {
                ...
                var idbatch = {
                    ...
                    textOrRedirectHandler: batch.textOrRedirectHandler,
                    path: batch.path,
                    handlers: [{
                        exceptionHandler: null,
        ...
        send2: function(batch) {
            ...
            if (batch.fileUpload) {
                ...
                if (isCrossDomain && !dwr.engine.isJaxerServer) {
        ...
        updateDwrSessionFromCookie: function() {
            if (!dwr.engine._dwrSessionId) {
                ...
                if (match) {
                    dwr.engine._debug("Found matching cookie");
                    dwr.engine.transport.setDwrSession(match[1])
                } else {
                    dwr.engine._debug("No matching cookie")
                }
            }
        },
        xhr: {
    ...
    dwr.engine.batch = {
        ...
        addCall: function(batch, scriptName, methodName, args) {
            ...
            if (lastArg == null || typeof lastArg == "function") {
                ...
                if (dwr.engine.util.isCallOptionArgument(lastArg)) {
                    callData = lastArg;
                    stopAt = args.length - 1
                } else {
                    callData = {};
            ...
            if (!batch.completed) {
                ...
                if (repliesReceived === 0) {
                    ...
                    if (repliesReceived < batch.map.callCount) {
                        dwr.engine._debug("Incomplete reply from server")
                    }
                }
            }
        },
        buildEmptyReplyException: function(batch) {
            ...
            return {
                name: "dwr.engine.emptyReply",
                message: "Empty reply from the server",
                status: status,
                htmlResponseText: responseText,
    ...
    dwr.engine.util = {
        ...
        isCallOptionArgument: function(lastArg) {
            return (typeof lastArg === "object" && (typeof lastArg.callback === "function" || typeof lastArg.exceptionHandler === "function" || typeof lastArg.callbackHandler === "function" || typeof lastArg.errorHandler === "function" || typeof lastArg.warningHandler === "function" || lastArg.hasOwnProperty("async")))
        }
    };
    dwr.engine._initializer.init();
    dwr.hub = {
